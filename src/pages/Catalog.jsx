...
function Catalog() {
    ...
    (async () => {
      try {
        ...
        allCategories.forEach((ct) =>
          console.log("➡️ Available:", ct.name.split(" ").join("-").toLowerCase())
        )
        ...
        if (matchedCategory) {
          ...
          console.log("✅ Matched categoryId:", matchedCategory._id)
        } else {
          console.log("❌ No matched category for catalogName:", catalogName)
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (categoryId) {
      console.log("📤 Calling getCatalogPageData with:", categoryId)
      ;(async () => {
        try {
